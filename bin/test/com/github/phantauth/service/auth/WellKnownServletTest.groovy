...
class WellKnownServletTest extends Specification {
    ...
    def "test openid-configuration"() {
        ...
        when:
        resp = instance.handleGet(new OIDCProviderConfigurationRequest(new Issuer(tenant.issuer)).toHTTPRequest())
    ...
    def "test oauth-authorization-server"() {
        ...
        when:
        resp = instance.handleGet(new AuthorizationServerConfigurationRequest(new Issuer(tenant.issuer)).toHTTPRequest())
