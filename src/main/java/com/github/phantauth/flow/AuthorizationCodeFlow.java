...
public class AuthorizationCodeFlow extends AbstractFlow {
    ...
    @Inject
    public AuthorizationCodeFlow(final TenantRepository tenantRepository, final Repository<User> userRepository, final UserTokenFactory userTokenFactory, final Repository<Client> clientRepository, final ClientTokenFactory clientTokenFactory) {
    ...
    boolean implied(final AuthorizationRequest request) {
        return request.getResponseType() != null && request.getResponseType().impliesCodeFlow();
    }
